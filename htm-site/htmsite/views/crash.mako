                ...
                <div class="panel-heading">
                    ...
                    <div class="pull-right">
                        <div class="btn-group-toggle" data-toggle="buttons">
                            <label class="btn btn-primary btn-sm " id="filter-label">
                                <input type="checkbox"  autocomplete="off" id="filter"> Filter
                            </label>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function anomalies2table(anomalies) {
        ...
        _.each(anomalies, function (a) {
            s += "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(a['site_no'], a['strategic_input'], a['ds'], a['algorithm'], moment(a['datetime'].$date).format("HH:mm"));
        });
        return s + "</body></table>";
    };
                    ...
                    $.post('/crashes',
                        ...
                        var ct = $('#crash-table');
                                ...
                                $.each(data['crashes'], function (i, v) {
                                    ...
                                    ct.append("<tr data-anom=\"{}\"><td>{}</td><td>{}</td><td>{}</td></tr>".format(hasAnom, moment(v['datetime']['$date']).local().format('ddd ll HH:mm'),
                                            v['Total_Damage'],
